...
int main(int argc, char** argv){
  ...
  if(Nmom != 2)
    ABORT("Error: compute_sigmapi needs Nmom to be 2\n");
  ...
  double *tmpMomP1P2 = (double*) malloc(2*info.Nt*info.NeV*info.NeV*2*sizeof(double));
...
#pragma omp parallel for default(none) shared(lapeig, tmp, nx, ny, nz, nt, px3, py3, pz3, ip, this_node) schedule(static)
    ...
    calculateSigmaPi(info,ptr_prop,tmpMomP1, tmpMomP2, tmpMomP1P2,Prec, outname);

  } // close loop over momenta
