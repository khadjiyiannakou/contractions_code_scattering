...
s1 = c_spinIndices_pipi_doubleTriangle_hor[bx%numSpinsComb][0];
s2 = c_spinIndices_pipi_doubleTriangle_hor[bx%numSpinsComb][1];
s3 = c_spinIndices_pipi_doubleTriangle_hor[bx%numSpinsComb][2];
s4 = c_spinIndices_pipi_doubleTriangle_hor[bx%numSpinsComb][3];

ti = bx / numSpinsComb;

coef.x = (FLOAT) c_coef_pipi_doubleTriangle_hor[bx%numSpinsComb][0];
coef.y = (FLOAT) c_coef_pipi_doubleTriangle_hor[bx%numSpinsComb][1];
...
//**************** Attention here we need the dagger *****************//
// ==================================================================//
#pragma unroll
 for(int ixb = 0 ; ixb < Nxb ; ixb++)
#pragma unroll
   for(int iyb = 0 ; iyb < Nyb ; iyb++){
     ...
     int aEnd   = aBegin + NSIZE - 1;
     int aStep  = BLOCK_SIZE;
     int bBegin = NSIZE * BLOCK_SIZE * ixb;   // we take the transpose over all the submatrices
	 ...
	 Bs[ty][tx] =  conj(FETCH_FLOAT2(texProp, offset_Prop + b + NSIZE * ty + tx));
...
#pragma unroll
	 for (int k = 0; k < BLOCK_SIZE; ++k)
	   {
	     Csub.x += As[ty][k].x * Bs[tx][k].x - As[ty][k].y * Bs[tx][k].y; // we take the transpose in the multiplication inside the submatrix
	     Csub.y += As[ty][k].x * Bs[tx][k].y + As[ty][k].y * Bs[tx][k].x;
	   }
	 __syncthreads();
       }
...
offset_Prop = ti*4*4*NSIZE*NSIZE +  s1*4*NSIZE*NSIZE + s4*NSIZE*NSIZE;
